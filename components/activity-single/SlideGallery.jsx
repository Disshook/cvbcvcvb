...
const SlideGallery = ({ files }) => {
  ...
  //   "/img/activities/11.png",
  // ];
  const sliderImg = files?.map((list, index) => list?.name);
          ...
          {sliderImg?.map((img, i) => (
            <SwiperSlide key={i}>
              <div className="ratio ratio-64:45">
                <img
                  src={`https://emt.tanuweb.cloud/uploads/${img}`}
                  alt="image"
          ...
          {sliderImg?.map((slide, i) => (
            ...
            >
              <Item
                width={1451}
                height={700}
                original={`https://emt.tanuweb.cloud/uploads/${slide}`}
                thumbnail={`https://emt.tanuweb.cloud/uploads/${slide}`}
              >
                {({ ref, open }) => (
                  <div
